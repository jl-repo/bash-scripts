...
echo -e "Checking certificates on sites with port 443."
echo -e "Grace days is set to $gracedays days."
echo -e ""
for server in "${servers[@]}"; do
        #Capture expiry date using openssl on port 443.
        data="$(date --date="$(echo | openssl s_client -connect "${server}":443 -servername "${server}" 2>/dev/null | openssl x509 -noout -enddate | awk -F '=' '{print $NF}' )" --iso-8601)"
